...
### 一、安全套件 (Security Suites)

DLMS 定义了三个安全套件，逐步增强加密能力：
...
- **Suite 0**：纯对称加密，仅使用 AES-GCM-128 + GMAC，是目前部署最广泛的方案（绝大多数 DLMS UA 认证的电表产品支持 Suite 0）
- **Suite 1**：在 Suite 0 基础上加入非对称密码学（ECDSA/ECDH P-256 + SHA-256），支持端到端保护和前向保密
- **Suite 2**：最强安全级别，使用 AES-256 + P-384 曲线 + SHA-384，面向高安全需求场景
