...
> [!NOTE]
> `.hpp`是函数声明和定义放一起的文件，是`.h`和`.cpp`的结合体，但不会被编译成.o，仍然是被include包含的文件。
...
> [!TIP]
> g++ `hello.cpp` -o hello -`stdc`++=11是指定使用c++11的标准编译。如果默认的编译器版本过高时，默认会舍弃了一些用法如`auto_ptr<T>`指针，而代码中用到了这个指针话，直接使用默认版本编译会报错，因此要使用支持该指针语法的编译器版本。

# 嵌套C语言

## 1.`extern "C"`

`extern "C"`起作用的时候是在C++调用C中的函数时候。由于C++和C是两种不同的编译和连接方法，所以在交叉调用的时候，就需要增加一些机制起到让两者无缝兼容的目的，而extern "C"正是这一种机制。规定在编译C++源文件时，那些调用自C文件的部分（需要用extern "C"修饰其在头文件中的声明），按照与C文件编译兼容的方式进行。
...
由于extern "C"修饰了`void add_fun(void)`，所以在`main.cpp`调用该函数的时候按照C文件的编译方式进行编译，所以其对应的标号为**add_fun**。add_fun函数所在的文件`add.c`文件编译出的标号与之一致，就能成功的连接。
...
### 3.引用所有成员
   ...
   ```
### 4.匿名空间引用
匿名空间用于限制变量、函数和类的作用域，使其仅在定义它们的文件中可见。这有助于减少全局命名空间的污染，提高代码的可读性和可维护性。

仅当前 `.cpp` 用到的辅助函数 / 变量 / 类放入匿名空间，避免污染全局命名空间。

通过使用匿名命名空间，可以实现文件局部性，同时保持代码的灵活性和可扩展性。
...
- C++的枚举类型是一个单独的数据类型，不是整型，不能当整型使用，如进行赋值（只有声明时才能赋值），自加自减等
...
**安全性**：编译时转换，相对安全，但不能在运行时检查类型转换的安全性。
...
引用的本质其实就是指针常量 :`<数据类型 >  *const p`，所以必须定义时赋初值，又因为指针常量指向哪个区域虽然不能改，但其所指向的区域的内容是可以改的。故其内容不能是常量，正因如此才不能对常量进行引用，因为常量不能改。
...
> [!CAUTION]
> 1.引用必须在定义时就初始化。
> 2.一般引用是对左值如变量的引用，不能对右值如常量、表达式进行引用。
...
函数的形参是引用，如果不希望函数内部的实现对形参进行改动，则形参应该为常引用，此时，实参既可以传变量这些左值也可以传常量这些右值。
...
> 3.函数的声明和实现不能同时去说明有默认参数，不然就报错。而默认参数的设计目标是 “对外接口的约定”，因此**必须写在声明处**（头文件），实现处绝对不能写
>
> ```c++
> int add(int x,int y ,int z = 100);
> int add(int x,int y ,int z =100){//error: default argument given for parameter 3 of ‘int add(int, int, int)’ 
>     return x + y + z;
> //默认参数是编译期特性（编译器在调用点自动填充默认值），而非运行时。如果声明和实现分别指定不同的默认参数，编译器无法确定该用哪一个.即使声明和实现的默认参数值相同，C++ 标准也禁止这种写法 —— 本质是为了避免 “接口与实现的默认参数不一致” 的潜在风险
> ```

## 2.哑元

哑元是只有类型没有变量名的参数。 也叫 “占位参数” 或 “空参数”，指的是**只有参数类型、没有参数名的函数参数**。它的核心作用是 “占位置”—— 既满足函数签名的语法要求，又不需要在函数体内使用这个参数主要用途如下：

- 兼容旧代码，保证函数的向下兼容性
  最常见的场景是**函数重载**—— 通过哑元区分不同的函数签名
...
> [!CAUTION]
> 1.形参变量名不同，不**构成**重载关系。
> 2.函数返回值类型不同，不构成重载关系的要素，即不**影响**重载关系。
...
2. **常量转换**
   ...
   ```c++
   忽略顶层 const/volatile
   const int 实参匹配 int 形参
   int 实参也匹配 const int 形参
   ```
3. **数组 / 函数退化为指针**

   ```c++
   int arr[5] 实参匹配 int* 形参，void func() 匹配 void (*)()
   ```

4. **哑元匹配**

   ```c++
   调用 func(10) 匹配 void func(int)
   ```
#### 2.升级转换

实参需进行**内置类型的 “提升” 转换**（无精度损失），优先级仅次于精确匹配。

```c++
char/unsigned char/short → int；
unsigned short → int（若 int 能容纳其值，否则→unsigned int）；
float → double；
布尔值 bool → int（false→0，true→1）。
```
#### 3.标准转换

实参需进行**内置类型的普通转换**（可能有精度损失），优先级低于提升。常见标准转换：

- 数值类型转换（如 `int→float`、`double→int`、`unsigned int→long`）；
- 指针转换（如 `int*→void*`、派生类指针→基类指针）；
- 整数 / 枚举类型转换（如 `enum E {A} → int`）；
- `NULL`/`nullptr` → 指针 / 布尔类型。

```c++
void print(float a) { cout << "float版本" << endl; }
void print(long a) { cout << "long版本" << endl; }

int main() {
    int x = 10;
    print(x);  // int→float 是标准转换，int→long 也是标准转换 → 二义性报错！
    return 0;
}
```
...
形参为省略号（`...`），匹配任意类型的实参（C 风格可变参数），优先级最低。

```c++
void print(int a) { cout << "int版本" << endl; }
void print(...) { cout << "省略号版本" << endl; }

int main() {
    char c = 'a';
    print(c);  // char→int 提升匹配，优先调用 print(int)
    print(10, 20);  // 无精确匹配，调用 print(...)
    return 0;
}
```
...
在 C++ 中，系统在执行函数调用时会进行一系列额外的操作，如保存和恢复现场环境、参数赋值等。这些操作对于短小且频繁调用的函数来说，可能会导致函数调用的开销超过函数体实际执行的时间，从而影响程序的整体性能。为了优化这种情况，C++ 引入了内联函数（`inline function`）的概念。不需要建立函数调用的运行环境的栈空间，不需要进行参数传递，不需要跳转，让执行效率更高。
...
内联函数的本质是将函数代码复制到函数调用的位置，这会增加程序的体积。因此，内联函数应尽量做到短小精悍，一般不要超过 5 行代码。如果将一个比较复杂的函数内联，往往会导致程序的体积迅速膨胀，最后可能得不偿失。

#### 2.内联函数执行的时间要短

内联函数不仅要简短，其执行时间也应尽可能短，以体现内联函数的优势。如果函数执行的时间远大于函数调用过程中所需的开销（即那些幕后工作所花费的时间），那么通过内联函数节省下来的那一点时间也就无足轻重了。
...
`inline `关键字只是对编译器的建议，表明程序员希望将函数内联处理。但是，编译器可以能会忽略这一建议，在某些特定的情况下将函数保持为普通函数，如函数体过大、包含循环 / 递归、虚函数等，并给出相应的警告。反过来，编译器在优化时也可能将未标记为` inline `的函数进行内联处理，只要编译器“认为”该函数内联会提高代码性能，也就是说，编译器也会自作主张地将函数进行内联处理。
...
- **封装**
  将数据和操作数据的函数包装在一起的主要目的就是实现封装和隐藏。隐藏就是不让结构体外的函数直接修改结构体中的数据，只能通过结构体的成员函数对数据进行修改。为了实现数据的隐藏，C++新增3三个访问权限限定符：
...
用虚析构函数做是为了当用一个基类的指针删除一个派生类的对象时，派生类的析构函数会被调用。当然，并不是要把所有类的析构函数都写成虚函数。因为当类里面有虚函数的时候，编译器会给类添加一个虚函数表，里面来存放虚函数指针，这样就会增加类的存储空间。所以，只有当一个类被用来作为基类的时候，才把析构函数写成虚函数。

在Base类的析构函数前加virtual。这样基类的虚函数就会被绑定到虚函数表中，那么就会得到调用，会**先调用派生类的析构函数，再调用基类的析构函数**，确保所有资源被释放。如果基类析构函数没加virtual关键字，那么子类的虚表中不会出现基类析构函数，子类的其他内存位置（要手动调用的地方）存的函数更不可能有基类的析构函数。
...
queue是一种专门为“先进先出”情形设计的容器，且只支持先进先出，只支持访问头尾的两个元素。

其实，queue本身并不是容器，而是对其它容器的一个封装。如下是queue的定义，是一个模板类，在具体化一个queue类型时，你需要声明这个queue将以那种容器作为内部的实现，如下图所示。可以看出，queue默认以std::deque作为内部实现。

![img](https://picx.zhimg.com/v2-f937493e7468a3eb89a3b53746a83045_1440w.jpg)

C++要求，任意支持至少以下6种操作的容器，都可以作为queue的内部实现类型。C++标准容器中，deque和list都满足此要求。

![img](https://pic4.zhimg.com/v2-584054c165bd506b345dd23ba5bbd171_1440w.jpg)

虽然默认情况下queue以deque作为内部实现，但queue不等于deque，绝对不能当作deque来使用，比如queue不支持访问非首尾元素，但duque是明确支持的。std::queue 是包装器——只有 push/pop/front，不能遍历、不能 clear()、不能 std::make_move_iterator 批量移出。

### 2.deque

std::deque 是实际容器——能遍历、能 swap、能 clear、能迭代器批量操作。drainAll() 需要整个队列原子的移出去，queue 做不到。

deque，也就是 **d**ouble-**e**nded **que**ue。deque非常强大，支持在双端进行增减操作，支持直接访问任意位置的元素，支持随机迭代器访问。

deque的底层实现是特定组织形式的[动态数组](https://zhida.zhihu.com/search?content_id=172807500&content_type=Article&match_order=1&q=动态数组&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3ODU0MDEyNDEsInEiOiLliqjmgIHmlbDnu4QiLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNzI4MDc1MDAsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MSwiemRfdG9rZW4iOm51bGx9.CSZIvS7yocyPJAqxnoZgKqXaW5nrJ_eyCguAtuGk1q8&zhida_source=entity)，能够根据需要动态地调整物理空间占用。deque与vector的对比非常有意思，两者底层都是数组，但对数组的使用却有本质不同，并造成了两者特性的差异。

vector底层是**一个**数组，动态拓展或动态缩减，是以申请新的内存空间并把数据移动到新内存空间上为代价，所有的元素始终在一个连续[物理空间](https://zhida.zhihu.com/search?content_id=172807500&content_type=Article&match_order=2&q=物理空间&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3ODU0MDEyNDEsInEiOiLniannkIbnqbrpl7QiLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNzI4MDc1MDAsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MiwiemRfdG9rZW4iOm51bGx9.7M4D21lDI1EHilnJqTIkhccEr-SbdGZx6aOQBFjMeu0&zhida_source=entity)上线性存放。这决定了vector的访问非常高效，同时尾部操作也非常高效。

deque的底层则是**若干个**数组的集合，单个数组内是连续物理空间，但不同数组间却不连续；deque的动态拓展或动态缩减，是通过新增或者释放物理空间片段来实现的，不发生数据的转移。deque内部维护了所有元素的必要信息，保证能够通过统一的接口直接访问所有的元素，且访问耗时相等，访问者无需关心各个元素是否位于同一个物理空间上。
