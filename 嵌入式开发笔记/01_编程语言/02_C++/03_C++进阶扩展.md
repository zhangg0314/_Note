...
- detach方式，启动的线程自主在后台运行，当前的代码继续往下执行，不等待新线程结束。
  ...
  ```txt
  在 C++ 里，std::thread::detach() 方法的作用是让线程对象和它所代表的实际执行线程分离。
  
  分离线程执行：
  	一旦调用了detach()，线程就会在后台独立运行。此时，线程对象和实际执行线程之间不再有关联，线程的资源会在执行完毕后自动释放。
  资源自动回收：
  	当分离的线程执行结束后，系统会自动回收它所占用的资源，无需主线程调用join()来等待。
  释放线程对象：
  	调用detach()之后，线程对象就可以被安全销毁了，不会影响到实际执行的线程。
  使用场景：
  	该方法适用于那些不需要等待完成的后台任务，像守护线程、日志记录线程等。
  ```
...
下面的代码，`join`后面的代码不会被执行，除非子线程结束。
如果需要获取线程的执行结果或者等待线程结束，应该使用`join()`而不是`detach()`。
...
分离线程时，要确保线程不会访问主线程中可能提前销毁的资源，防止出现悬空引用。
...
| sleep_until | void sleep_until(const std::chrono::time_point<Clock, Duration>& sleep_time); | 暂停直到某个时刻                            |
...
2. 因为涉及到共同资源g_i ，所以需要一个共同`mutex：g_i_mutex`。
...
- `std::mutex m`;在结构体中，`mutex`不是共享的。但是只需要一把锁也能锁住，因为引用传递后，同一把锁传给了两个函数。
...
`condition_variable`必须结合`unique_lock`使用。`condition_variable_any`可以使用任何的锁。下面以condition_variable为例进行介绍。

`condition_variable`条件变量可以阻塞（`wait`、`wait_for`、`wait_unti`）调用的线程直到使用（`notify_one`或`notify_al`l）通知恢复为止。`condition_variable`是一个类，这个类既有构造函数也有析构函数，使用时需要构造对应的`condition_variable`对象，调用对象相应的函数来实现上面的功能。
...
public:
    ...
    std::queue<T*> tasks_queue; //任务队列 - 修正模板参数
    bool append(T *request);//往请求队列＜task_queue＞中添加任务
private:

    //工作线程需要运行的函数,不断的从任务队列中取出并执行
    static void worker(void *arg); // 修正返回类型为void
    void run();
private:
...
//构造函数，创建线程
template <typename T>
threadPool<T>::threadPool(int number) : stop(false) // 修正构造函数作用域
{
    ...
    for (int i = 0; i < number; i++)
    {
        ...
        // 使用lambda包装worker函数，确保正确传递this指针
        work_threads.emplace_back([this]() { this->worker(this); });
    }
}

template <typename T>
inline threadPool<T>::~threadPool() // 修正析构函数作用域
{
    {
...
//添加任务
template <typename T>
bool threadPool<T>::append(T *request) // 修正函数作用域
{
...
//单个线程
template <typename T>
void threadPool<T>::worker(void *arg) // 修正函数作用域和返回类型
{
    threadPool<T>* pool = static_cast<threadPool<T>*>(arg); // 修正类型转换
    pool->run();//线程运行
    // 移除返回语句
}

template <typename T>
void threadPool<T>::run() // 修正函数作用域
{    
    while (!stop)
    {
        std::unique_lock<std::mutex> lk(this->queue_mutex);
        /*　unique_lock() 出作用域会自动解锁　*/
        this->condition.wait(lk, [this] { return !this->tasks_queue.empty() || stop; }); // 增加stop条件
...
`C++中`的`std::atomic`是C++11引入的标准库组件，位于`<atomic>`头文件中，用于提供原子操作，解决多线程环境下的数据竞争问题。 原子操作是不可分割的操作，在执行完毕前不会被其他线程中断。使用`std::atomic`可以避免使用互斥锁，从而提高性能。
...
1. **构造函数** 
   ...
   ```c++
   std::atomic<T> var;             // 默认构造，未初始化
   std::atomic<T> var(value);      // 初始化为 value
   std::atomic<T> var = value;     // 同上
   ```
...
2. **store () - 写入操作**
   ...
   //功能：原子地将值 desired 存储到原子对象中
   //参数：
   	-desired：要存储的值
   	-order：内存序约束（默认为顺序一致性）
           
   std::atomic<int> counter(0);
   counter.store(42); // 原子地将 counter 设置为 42
   ```
...
3. **load () - 读取操作**
   ...
   int value = counter.load(); // 原子地读取 counter 的值
   ```
...
4. **exchange () - 交换操作**
   ...
   //原子地将原子对象的值替换为 desired，并返回旧值
   
   int old_value = counter.exchange(100); // 将 counter 设为 100，返回旧值
   ```

5. **compare_exchange_weak () /compare_exchange_strong () - 比较交换操作**

   ```c++
   bool compare_exchange_weak(T& expected, T desired,std::memory_order success, std::memory_order failure);
   bool compare_exchange_strong(T& expected, T desired, std::memory_order success,std::memory_order failure);
   
   //功能：如果原子对象的值等于 expected，则将其替换为 desired, 如果不等于，则将 expected 更新为当前值
   //区别：
   	-weak 版本可能会虚假失败（即使值相等也可能返回 false）
   	-strong 版本保证不会虚假失败
   ...
   if (success) {
       // counter 原来等于 42，现在被设置为 100
   } else {
       // counter 不等于 42，expected 现在包含 counter 的当前值
   }
   ```
...
6. **算术（仅适用于整型和指针类型）**
   ...
   ```c++
    //fetch_add () /fetch_sub () - 原子增减操作
   T fetch_add(T arg, std::memory_order order = std::memory_order_seq_cst);
   T fetch_sub(T arg, std::memory_order order = std::memory_order_seq_cst);
   
   //功能：原子地将 arg 加到 / 从原子对象的值中减去，并返回旧值
   
   int old_value = counter.fetch_add(1); // 等价于 counter++，但返回旧值
   int new_value = counter + 1;          // 等价于 fetch_add 后再加 1
   ```

###  5.内存序 

在多线程编程中，编译器和处理器为了优化性能，可能会对指令进行**重排序**：

- **编译器重排序**：编译器在不改变单线程行为的前提下重新安排指令顺序
- **处理器重排序**：处理器执行指令的顺序可能与程序顺序不同

内存序提供了一种机制，让我们可以控制这种重排序，确保线程间的内存可见性。

`std::atomic`提供6种内存序模型，最常用的是:
...
std::memory_order_seq_cst // 顺序一致性内存序（默认）
```
...
`log4cplus`是用C++编写的[开源](https://edu.csdn.net/cloud/pm_summit?utm_source=blogglc&spm=1001.2101.3001.7020)的日志系统，前身是`java`编写的`log4j`系统，受Apache Software License保护，作者是Tad E. Smith。`log4cplus`具有线程安全、灵活、以及多粒度控制的特点，通过将日志划分优先级使其可以面向程序调试、运行、测试、和维护等全生命周期。且其可以选择将日志输出到屏幕、文件、NT event log、甚至是远程服务器；并通过指定策略对日志进行定期备份等等。

[一个著名的日志系统是怎么设计出来的？](https://mp.weixin.qq.com/s/XiCky-Z8-n4vqItJVHjDIg)

[log4cplus - log4cplus的设计 - 《C 第三方库》 - 极客文档](https://geekdaxue.co/read/softdev@cpplibs/cqhyw1)
...
og4cplus中的嵌入诊断上下文（Nested Diagnostic Context），即NDC。

对log系统而言，当输入源可能不止一个，而只有一个输出时，往往需要分辩所要输出消息的来源，比如服务器处理来自不同客户端的消息时就需要作此判断，NDC可以为交错显示的信息打上一个标记(stamp)，使得辨认工作看起来比较容易些。

这个标记是线程特有的，利用了线程局部存储机制，称为线程私有数据（Thread-Specific Data，或TSD）。

## 6.基本使用步骤

1. 实例化一个封装了输出介质的appender对象；
2. 实例化一个封装了输出格式的layout对象；
3. 将layout对象绑定(attach)到appender对象；如省略此步骤，默认是简单布局器SimpleLayout对象会绑定到logger。
...
|       选项       |                             作用                             |
| :--------------: | :----------------------------------------------------------: |
| `ImmediateFlush` | 是否立即刷新（默认为true）log4cplus.appender.XXX.ImmediateFlush=true |
|       File       |   使用的日志文件名log4cplus.appender.XXX.File=all_msgs.log   |
|      Append      |    是否追加到之前的文件log4cplus.appender.xxx.Append=true    |
|  `ReopenDelay`   | 先将日志缓存起来，等指定时间之后再往文件中插入，减少文件的保存次数。log4cplus.appender.xxx.ReopenDelay=10【单位为秒】 |
|  `UseLockFile`   | 是否使用加锁的方式去写文件，默认是falselog4cplus.appender.xxx.UseLockFile=true |
|    `LockFile`    | 使用的加锁文件名log4cplus.appender.xxx.LockFile=xxx.lock[文件名没有具体要求] |
|      Locale      | 使用的字符集log4cplus.appender.xxx.Locale=chs【en，其他参数具体见imbue参数】 |
|    Threshold     | 指定日志消息的输出最低层次log4cplus.appender.xxx.Threshold=DEBUG |
...
```
  ...
  print(siNum)，siNum 是 short。两个候选：

  ┌───────────────────┬───────┬────────────────────────────┐
  │       候选        │ 参数  │            匹配            │
  ├───────────────────┼───────┼────────────────────────────┤
  │ 非模板 print(int) │ int   │ short→int 整型提升，非精确 │
  ├───────────────────┼───────┼────────────────────────────┤
  │ 模板 print(_Ty)   │ short │ short→short 精确匹配       │
  └───────────────────┴───────┴────────────────────────────┘

  模板精确匹配赢。所以选中模板，_Ty 推成 short。

  然后炸

  选中模板后，要实例化函数体：

  typename short::value_type vt_someval;   // short 没有 value_type！

  short 没有 value_type → 硬错误。

  为啥不是 SFINAE 淘汰

  SFINAE 只管签名（参数、返回类型），叫"立即上下文"。不管函数体。

  - 上一个例子里 value_type* 在参数里 → 替换失败 → SFINAE 淘汰模板。
  - 这个例子里 value_type 在函数体里 → 签名本身合法（void print(short) 没问题）→ 模板活过重载决议 → 实例化函数体时才炸。

  顺序是：先选中（签名合法）→ 再实例化函数体 → 函数体里出错 → 报错。SFINAE 救不了函数体。

  正确写法

  想让它 SFINAE 生效，把 value_type 挪到签名里：

  template < typename _Ty >
  void print( _Ty tt, typename _Ty::value_type* = NULL ){   // 挪到参数里
        cout<<"template < typename _Ty >"<< endl;
  }

  这样 short 没有 value_type → 参数替换失败 → 模板被 SFINAE 淘汰 → 只剩非模板 print(int) → 输出 void print( int )。

  一句话

  ▎ SFINAE 只作用在签名，不作用在函数体。value_type 写函数体里 = 晚了，重载决议已经选完你，实例化时直接硬报错。

  要我把"立即上下文"到底包含哪些位置（参数、返回类型、模板参数列表……）列清楚吗？这是 SFINAE 最容易翻车的地方。    
...
// 定义不同类型的任务数据
struct ObisData {
    std::vector<ObisCode> obis_list;
};

struct ApduData {
    BNDLMS::CGXByteBuffer Apdu;  // 注意：这里的类型名可能有误
};
// 使用模板和组合
...
struct Task {
    ...
    std::vector<std::string> meter_addresses;
    ...
    Task(int id, TaskType tasktype, TaskPriority prio, bool en,
                  const std::chrono::system_clock::time_point& start,
                  const std::chrono::system_clock::time_point& end,
                  std::chrono::seconds intv, int retry, std::chrono::seconds retry_intv)
        : task_id(id), task_type(tasktype), priority(prio), enabled(en), start_time(start),
          end_time(end), interval(intv), retry_count(retry),
          retry_interval(retry_intv), next_execution(start) {}
    ...
    bool operator==(const Task& other) const {
        return task_id == other.task_id &&
               ...
               meter_addresses == other.meter_addresses;
    }
...
// 使用别名简化类型定义
using ScheduledTask = Task<ObisData>;// 定时抄表任务定义
using BatchParamsSetTask = Task<ApduData>;
...
[线程'std::system_error‘什么()操作不被允许-腾讯云开发者社区-腾讯云](https://cloud.tencent.com/developer/ask/sof/115522648/answer/131148724)

```c++
std::unique_lock<std::mutex> lock1(BatchTaskProcessor::GetInstance().mutex_); // 与处理批量任务的线程互斥
if (BatchTaskProcessor::GetInstance().batchtask_work_queue_.empty())
    return false;
std::unique_lock<std::mutex> lock2(channels_mutex_); // 与处理其他任务的线程互斥

// 获取当前活跃的电表集合
std::unordered_set<std::string> active_meters = getCurrentMeters();

// 查找第一个可用的工作项（电表不在活跃状态且重试时间已到）
while (!BatchTaskProcessor::GetInstance().batchtask_work_queue_.empty())
{
    auto top_item = BatchTaskProcessor::GetInstance().batchtask_work_queue_.top();
    BatchTaskProcessor::GetInstance().batchtask_work_queue_.pop();

    // 检查电表是否活跃以及是否到了重试时间,active_meters相当于临界资源的备份，此时还不能解锁
    if (active_meters.find(top_item->meter_address) == active_meters.end() &&
        (top_item->current_retry > 0 || now >= top_item->retry_time))
    {
        batch_work_item = top_item;
        found = true;
        auto &channel = channels_[channel_id];
        channel->setState(ChannelState::REALTIME);
        channel->setCurrentMeter(batch_work_item->meter_address);
        lock2.unlock(); //xxxxxxxxxxxxxxxxxxxxxxx
        break;
    }
    // 将不满足条件的工作项放入临时队列
    temp_queue.push(top_item);
    lock2.unlock();//xxxxxxxxxxxxxxxxxxxxxx，多次解锁，解锁一个未上锁的锁！！！！Error
}
// 将临时队列中的工作项放回原队列
while (!temp_queue.empty())
{
    BatchTaskProcessor::GetInstance().batchtask_work_queue_.push(temp_queue.top());
    temp_queue.pop();
}
```