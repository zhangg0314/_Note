...
5. vim **Makefile**   （**自己编写或者公司提供的辅助makefile,位于其他目录**）
   ...
   ​        虽然是不同目录，但make  Linux顶层目录的Makefile也同其他位于同一目录下的模块文件一样的原理，只不过要额外自己编写一个makefile来协助顶层目录的makefile，比如告诉Linux内核的顶层Makefile模块源码目录的路径。而第二次执行这个自己编写的协助的makefile时相当于执行模块源码目录下的Makefile本来就要有的内容`obj-m += hello.o`,即同一目录下的模块源文件的Makefile也要有这一行生成.o的命令。
...
6. `make`（生成的`ko`文件适用于主机ubuntu linux）
