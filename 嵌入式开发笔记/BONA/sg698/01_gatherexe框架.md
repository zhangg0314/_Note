...
通信方式是指集中器下行的通信模块所采用的通信方式类型，不同的通信方式决定用户数据区中的数据构成和格式，主要有下面这些通信方式。     
...
- `gatherexe`进程中创建了一个`dlt698_42`线程，他们之间通过`udpskt`（`UDP`套接字）来进行通信。主进程赋值监听消息队列或者检查抄表任务配置数据库来看是否有抄表任务需要执行，然后做进一步处理，通过`UDP`套接字发给线程，由线程来执行。
- `dlt698_42`线程通过`select`函数监听来自上行的`gathexe`进程的消息和来自下行的串口消息。下行串口可以是485串口也可以是载波模块的串口。
