...
虚函数是实现多态的基础。被virtual关键字修饰的成员函数称为虚函数。
...
在Base类的析构函数前加virtual.这样基类的虚函数就会被绑定到虚函数表中，那么就会得到调用。如果基类析构函数没加virtual关键字，那么子类的需表中不会出现基类析构函数，子类的其他内存位置（要手动调用的地方）存的函数更不可能有基类的析构函数
