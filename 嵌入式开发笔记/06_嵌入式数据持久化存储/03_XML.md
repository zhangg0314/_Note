# XML简介

XML本质就是一个文件，文件在任何系统都是兼容可用的。XML 被设计用来传输和存储数据，而HTML 被设计用来显示数据。XML 指可扩展标记语言（eXtensible Markup Language）。可扩展标记语言（英语：Extensible Markup Language，简称：XML）是一种标记语言，是从标准通用标记语言（SGML）中简化修改出来的。它主要用到的有可扩展标记语言、可扩展样式语言（XSL）、XBRL和XPath等。

- XML 指可扩展标记语言（EXtensible Markup Language）。
- XML 是一种很像HTML的标记语言。
- XML 的设计宗旨是传输数据，而不是显示数据。
- XML 标签没有被预定义。您需要自行定义标签。
- XML 被设计为具有自我描述性。
- XML 是 W3C 的推荐标准。

------

# XML用途

## 1.不会做任何事

XML 不会做任何事情。XML 被设计用来结构化、存储以及传输信息。下面实例是 Jani 写给 Tove 的便签，存储为 XML：

```xml
<note> 
    <to>Tove</to> 
    <from>Jani</from> 
    <heading>Reminder</heading> 
    <body>Don't forget me this weekend!</body> 
</note>
```

上面的这条便签具有自我描述性。它包含了发送者和接受者的信息，同时拥有标题以及消息主体。但是，这个 XML 文档仍然没有做任何事情。它仅仅是包装在 XML 标签中的纯粹的信息。我们需要编写软件或者程序，才能传送、接收和显示出这个文档。

## 2.把数据从 HTML 分离

如果需要在 HTML 文档中显示动态数据，那么每当数据改变时将花费大量的时间来编辑 HTML。通过 XML，数据能够存储在独立的 XML 文件中。这样就可以专注于使用 HTML/CSS 进行显示和布局，并确保修改底层数据不再需要对 HTML 进行任何的改变。通过使用几行 JavaScript 代码，就可以读取一个外部 XML 文件，并更新网页的数据内容。

## 3.简化数据共享

在真实的世界中，计算机系统和数据使用不兼容的格式来存储数据。XML 数据以纯文本格式进行存储，因此提供了一种独立于软件和硬件的数据存储方法。这让创建不同应用程序可以共享的数据变得更加容易。

## 4.简化数据传输

对开发人员来说，其中一项最费时的挑战一直是在互联网上的不兼容系统之间交换数据。由于可以通过各种不兼容的应用程序来读取数据，以 XML 交换数据降低了这种复杂性。

## 5.简化平台变更

升级到新的系统（硬件或软件平台），总是非常费时的。必须转换大量的数据，不兼容的数据经常会丢失。XML 数据以文本格式存储。这使得 XML 在不损失数据的情况下，更容易扩展或升级到新的操作系统、新的应用程序或新的浏览器。

## 6.使数据更有用

不同的应用程序都能够访问数据，不仅仅在 HTML 页中，也可以从 XML 数据源中进行访问。通过 XML，数据可供各种阅读设备使用（掌上计算机、语音设备、新闻阅读器等），还可以供盲人或其他残障人士使用。

## 7.用于创建新的互联网语言

很多新的互联网语言是通过 XML 创建的。这里有一些实例：

- XHTML
- 用于描述可用的 Web 服务 的 WSDL
- 作为手持设备的标记语言的 WAP 和 WML
- 用于新闻 feed 的 RSS 语言
- 描述资本和本体的 RDF 和 OWL
- 用于描述针对 Web 的多媒体 的 SMIL

# XML与数据库对比

XML和数据库在存储数据的应用场景上有显著差异，核心区别在于**XML更适合轻量、临时性或需跨系统交换的数据**，而**数据库则专注于大规模、结构化、需高效管理和查询的数据**。以下从具体场景出发，详细说明两者的适用范围：

## 1.XML 存储数据的典型应用场景

XML 作为一种标记语言，其优势在于**格式自描述、跨平台兼容、结构灵活**，因此更适合以下场景：

#### 1. 配置文件存储

- **场景特点**：数据量小、结构可能频繁微调、需要人类可读且易于手动修改。
- 举例：
  - 软件系统的配置参数（如 Tomcat 的`server.xml`定义端口、虚拟主机；Maven 的`pom.xml`管理依赖）；
  - 设备或程序的初始化设置（如打印机的参数配置、游戏的用户偏好设置）。
- **原因**：XML 的树形结构能清晰表达层级关系（如 “父配置→子配置”），且无需复杂语法即可修改，适合非开发人员偶尔调整。

#### 2. 跨系统数据交换

- **场景特点**：不同系统（可能跨语言、跨平台）需要共享数据，需统一数据格式以避免歧义。
- 举例：
  - 企业间的业务数据交换（如电商平台与物流系统的订单信息同步，用 XML 标记订单号、商品、收件人等）；
  - 早期 Web 服务（如 SOAP 协议）的数据传输格式（通过 XML 封装请求和响应）；
  - 政府、金融机构的报表数据上报（用标准 XML 格式确保不同部门能解析）。
- **原因**：XML 是国际标准，标签自描述（如`<orderId>`明确表示订单号），无关系统只需遵循格式即可互认，无需依赖特定数据库类型。

#### 3. 轻量型本地数据存储

- **场景特点**：数据量极小（通常几千条以内）、无需复杂查询，仅需简单的持久化和读取。
- 举例：
  - 个人通讯录的本地备份（用 XML 记录联系人姓名、电话、邮箱的层级关系）；
  - 小型工具的日志文件（如程序运行的步骤、时间戳，用 XML 按时间顺序嵌套存储）。
- **原因**：无需安装数据库软件，直接以文本文件存储，适合个人或小型工具的简单需求。

#### 4. 文档型数据存储

- **场景特点**：数据本身是 “文档”（如报告、简历），包含富文本、排版信息或不规则结构。
- 举例：
  - 电子书的元数据（如 EPUB 格式的图书信息，用 XML 描述书名、作者、章节结构）；
  - 公文、合同的电子存档（用 XML 标记标题、正文、签章等部分，保留格式信息）。
- **原因**：XML 的嵌套结构能完美映射文档的层级（如 “章→节→段落”），且可嵌入样式信息（如`<bold>`标签表示加粗）。

## 2.典型应用场景

数据库的核心优势是**高效管理大规模数据、支持复杂查询、保障数据一致性**，因此适合以下场景：

#### 1. 业务系统的核心数据存储

- **场景特点**：数据量大（几万到上亿条）、需高频读写、多用户并发访问、有严格的事务和完整性要求。
- 举例：
  - 电商平台：用户信息、商品库存、订单记录（需支持 “下单时扣减库存” 等事务操作，避免超卖）；
  - 银行系统：账户余额、交易流水（需确保数据不丢失、不重复，支持按账号查询历史交易）；
  - 企业 ERP 系统：采购、销售、库存数据（需多表关联查询，如 “查询某客户近 3 个月的所有订单”）。
- **原因**：数据库通过索引、事务、锁机制等保障高并发下的效率和准确性，这是 XML 无法实现的。

#### 2. 数据分析与报表生成

- **场景特点**：需对大量历史数据进行统计、聚合、多维度分析。
- 举例：
  - 电商平台的销售报表：按地区、商品类别统计月销量，计算同比增长率；
  - 教育系统的成绩分析：按班级、科目统计平均分，筛选不及格学生。
- **原因**：数据库的 SQL 语言支持复杂查询（如`GROUP BY`分组、`JOIN`关联、子查询），且有查询优化器提升效率，而 XML 用 XPath/XQuery 处理大规模数据会非常缓慢。

#### 3. 多用户共享与协作数据

- **场景特点**：多人同时操作同一批数据，需控制权限、记录修改痕迹。
- 举例：
  - 协同办公系统（OA）：请假单、审批流程（不同用户查看、修改自己的单据，需权限控制）；
  - 项目管理工具：任务列表、进度跟踪（团队成员实时更新任务状态，需避免冲突）。
- **原因**：数据库支持用户权限管理（如仅管理员可删除数据）、事务隔离（避免两人同时修改同一数据导致冲突），而 XML 文件直接共享易出现覆盖、丢失问题。

#### 4. 高可用性与持久化存储

- **场景特点**：数据需长期保存，且需支持备份、恢复、容灾。
- 举例：
  - 医疗系统的患者病历：需长期存储，支持按身份证号查询，且不能丢失（法律要求）；
  - 政务系统的居民信息：需跨部门共享，支持容灾备份（如主从数据库同步）。
- **原因**：数据库提供完善的备份策略（如定时快照、日志备份）、故障恢复机制，而 XML 文件需手动管理备份，可靠性低。

## 3.总结

- **选 XML**：当数据需**跨系统交换**、**轻量存储**、**结构灵活且人类可读**，且数据量小、无需复杂查询时（如配置文件、小文件交换）。
- **选数据库**：当数据需**大规模存储**、**高频读写**、**复杂查询**、**多用户并发访问**或**事务保障**时（如业务系统、数据分析）。

实际应用中，两者常结合使用：例如，数据库存储核心业务数据，需要对外交换时，将数据从数据库导出为 XML 格式传输，接收方再解析 XML 并写入自己的数据库。

------

# XML格式

```xml
<?xml version="1.0" encoding="utf-8" ?> <!--这行是必须要写的，且必须放在首行（前面有注释都不行）。表示版本为1.0，以utf-8字符集来编码-->
 
<!--Users是一个根标签，必须只能有一个，而Users里面的子属性可以有多个,根标签只能有一个，子标签可以有多个-->
<!--根标签名字随意取,标签是成对存在的，记得嵌套正确-->
<Users>
    <user id="1"><!--id表示user节点的属性-->
        <name>张三</name>
        <age>18</age>
        <address>广州</address>
    </user>
    <userAttribute>都是爱学习的人</userAttribute>
    <user id="2">
        <name>李四</name>
        <age>25</age>
        <address>哈尔滨</address>
    </user>
 
    <!--以下是带有大于号小于号等特殊字符的写法-->
    <special>
        <![CDATA[ 5 > 2 && 3 < 5 ]]>
    </special>
    <!--特殊字符用法二-->
    <special>  5 &gt; 2 &amp;&amp; 3 &lt; 5 </special>
</Users>
```

![image-20250611154345101](..\figure\image-20250611154345101.png)

------

# ——XML文件解析——

# 1.创建XML文件

tinyxml使用的是DOM解析方式，优点是解析速度快，缺点是无法解析复杂的xml文件。

## 1.创建一个XML类

```c++
    // 创建一个XML类
    TiXmlDocument *tinyXmlDoc = new TiXmlDocument();
    if (tinyXmlDoc == nullptr)
        printf("创建XML文件失败\n");
    else
        printf("创建XML文件成功\n");
```

## 2.创建头部信息

```c++
    //创建头部信息
    TiXmlDeclaration *declare = new TiXmlDeclaration("1.0", "utf-8", "");
    // 插入文档类中
    if(tinyXmlDoc->LinkEndChild(declare) == NULL)
        printf("%s\n",tinyXmlDoc->ErrorDesc());
    else
        printf("插入声明成功\n");
```

```shell
root@cqr40:~# ./app
创建XML文件成功
插入声明成功
<?xml version="1.0" encoding="utf-8" ?>
```

## 3.创建根节点

```c++
// 创建时需要指定根节点的名称
TiXmlElement* Library = new TiXmlElement("Library");
tinyXmlDoc->LinkEndChild(Library);		// 把根节点插入到文档类中
```

```bash
创建XML文件成功
插入声明成功
<?xml version="1.0" encoding="utf-8" ?>
<Library />
```

## 4.添加子节点

首先创建一个子节点，然后新建一个文本，将文本添加到子节点中，最后将字节点添加到根节点中。

```c++
TiXmlElement *Book = new TiXmlElement("Book");
TiXmlText *bookText = new TiXmlText("书本");	// 创建文本
Book->LinkEndChild(bookText);	// 给Book节点添加文本
Library->LinkEndChild(Book);	// 插入到根节点下
```

```shell
创建XML文件成功
插入声明成功
<?xml version="1.0" encoding="utf-8" ?>
<Library>
    <Book>书本</Book>
</Library>
```

## 5.添加带属性的子节点

使用setAttribute可以简单的给节点添加属性值；如果要创建子节点的子节点，就跟创建子节点一样，只是创建好后要添加到子节点中即可。

```c++
    TiXmlElement *Book = new TiXmlElement("Book");
    // 插入属性
    Book->SetAttribute("ID", 1);
    Book->SetAttribute("Name", "水浒传");
    Book->SetAttribute("Price", "64.6");

    TiXmlText *bookText = new TiXmlText("书本"); // 创建文本
    Book->LinkEndChild(bookText);                // 给Book节点添加文本
    node1->LinkEndChild(Book);                   // 插入到根节点下
```

```bash
<Library>
    <Book ID="1" Name="水浒传" Price="64.6">书本</Book>
</Library>
```

## 6.将doc写入xml文件

只需使用文档类调用SaveFile方法即可，参数传文件名,写入成功返回true，写入失败返回false

```c++

    // 如果只是想在控制台打印而已，那么可以直接调用函数tinyXmlDoc->Print();就可以实现在控制台上打印出来了！
    // tinyXmlDoc->Print();
    bool result = tinyXmlDoc->SaveFile("test.xml");
    if (!result)
        printf("%s\n", tinyXmlDoc->ErrorDesc());
    else
        printf("创建XML文件成功\n");
    return 0;
```

```bash
root@cqr40:~# ./app
创建XML文件成功
插入声明成功
创建XML文件成功
root@cqr40:~# ls
app  test.xml
root@cqr40:~# 
```

## 7.转换为字符串

```c++
TiXmlPrinter printer;
tinyXmlDoc->Accept(&printer);
printf("%s\n", printer.CStr());


//如果只是想在控制台打印而已，那么可以直接调用函数tinyXmlDoc->Print();就可以实现在控制台上打印出来了！
tinyXmlDoc->Print();
```

## 8.示例代码

```c++
#include <stdio.h>
#include <cstddef>
#include "tinyxml.h"
#include "tinystr.h"
int main(int argc, char *argv[])
{
    // 创建一个XML类
    TiXmlDocument *tinyXmlDoc = new TiXmlDocument();
    if (tinyXmlDoc == NULL)
        printf("创建XML文件失败\n");
    else
        printf("创建XML文件成功\n");

    // 创建头部信息
    TiXmlDeclaration *declare = new TiXmlDeclaration("1.0", "utf-8", "");
    // 插入文档类中
    if (tinyXmlDoc->LinkEndChild(declare) == NULL)
        printf("%s\n", tinyXmlDoc->ErrorDesc());
    else
        printf("插入声明成功\n");

    // 创建根节点
    //  创建时需要指定根节点的名称
    TiXmlElement *node1 = new TiXmlElement("Library");
    tinyXmlDoc->LinkEndChild(node1); // 把根节点插入到文档类中

    TiXmlElement *Book = new TiXmlElement("Book");

    // 插入属性
    Book->SetAttribute("ID", 1);
    Book->SetAttribute("Name", "水浒传");
    Book->SetAttribute("Price", "64.6");

    TiXmlText *bookText = new TiXmlText("书本"); // 创建文本
    Book->LinkEndChild(bookText);                // 给Book节点添加文本
    node1->LinkEndChild(Book);                   // 插入到根节点下

    // 如果只是想在控制台打印而已，那么可以直接调用函数tinyXmlDoc->Print();就可以实现在控制台上打印出来了！
    // tinyXmlDoc->Print();
    bool result = tinyXmlDoc->SaveFile("test.xml");
    if (!result)
        printf("%s\n", tinyXmlDoc->ErrorDesc());
    else
        printf("创建XML文件成功\n");

    TiXmlPrinter printer;
    tinyXmlDoc->Accept(&printer);
    printf("%s\n", printer.CStr());
    return 0;
}
```

------

# 2.解析XML文件

## 1.导入文件

导入文件并读取文件中的xml内容，然后初始化它，这里有两种方式。

- **方式1**

  ```c++
  int main(int argc, char *argv[])
  {
  
      // 定义一个TiXmlDocument类指针
      TiXmlDocument *tinyXmlDoc = new TiXmlDocument;
      // 读取文件中的xml
      if (!tinyXmlDoc->LoadFile("oopmenu.xml"))
      {
          // 读取失败，打印失败原因
          printf("Could not load example xml file %s. Error='%s'\n", "oopmenu.xml", tinyXmlDoc->ErrorDesc());
          return -1;
      }
      else
          printf("导入文件成功!!!\n");
      return 0;
  }
  ```

  ```bash
  root@cqr40:~# ./app
  导入文件成功!!!
  root@cqr40:~# 
  ```

- **方式2**
  定义xml文档类对象时指定文件名，然后在使用这个对象去调用LoadFile传参TIXML_ENCODING_LEGACY等三个枚举值即可。

  ```c++
      TiXmlDocument *tinyXmlDoc = new TiXmlDocument("oopmenu.xml");
      // 然后在使用这个对象去调用LoadFile传参TIXML_ENCODING_LEGACY即可
      if (!tinyXmlDoc->LoadFile(TIXML_ENCODING_LEGACY))
      {
          printf("Could not load example xml file %s. Error='%s'\n", "oopmenu.xml", tinyXmlDoc->ErrorDesc());
          return -1;
      }
      else
          printf("导入文件成功!!!\n");
  ```

  ```bash
  root@cqr40:~# ./app
  导入文件成功!!!
  root@cqr40:~# 

## 2.获取头部信息

```c++

    // 读取文档声明信息(也就是xml的头部信息：<?xml version="1.0" encoding="utf-8" ?>)
    TiXmlDeclaration *pDeclar = tinyXmlDoc->FirstChild()->ToDeclaration();
    if (pDeclar != NULL)
    {
        printf("头部信息： version is %s , encoding is %s\n", pDeclar->Version(), pDeclar->Encoding());
    }
```

```bash
root@cqr40:~# ./app
导入文件成功!!!
头部信息： version is 1.0 , encoding is GB2312
```

## 3.获取根节点

这个定义不一定是需要指定参数根节点的名字，然后再通过RootElement方法获取。

```c++
    // 读取文档声明信息(也就是xml的头部信息：<?xml version="1.0" encoding="utf-8" ?>)
    TiXmlDeclaration *pDeclar = tinyXmlDoc->FirstChild()->ToDeclaration();
    if (pDeclar != NULL)
    {
        printf("头部信息： version is %s , encoding is %s\n", pDeclar->Version(), pDeclar->Encoding());
    }

    TiXmlElement *Library =  tinyXmlDoc->RootElement();
    if(Library == NULL)
        printf("%s\n",tinyXmlDoc->ErrorDesc());
    else
        printf("获取根节点成功\n");
    //Library =
    return 0;
```

```bash
导入文件成功!!!
头部信息： version is 1.0 , encoding is GB2312
获取根节点成功
root@cqr40:~# 
```

## 4.删除

### 1.删除子节点

/* 删除book 和 其中一个book1节点 和 其中一个属性 */

1. 定义一个xml文件类，并读取文件中的xml内容初始化它

2. 获取根节点
  这个定义是需要指定参数根节点的名字，然后再通过RootElement方法获取。

  ```c++
  // 创建时需要指定名称
  TiXmlElement *Library = new TiXmlElement("Library");
  Library = tinyXmlDoc->RootElement();
  ```

3. 删除Book节点
  像这种只有自己本身的，没有其他子节点的节点，它有两种方式去删除。

  ```c++
  //1). 方式一
  //获取到对应节点后调用Clear()方法去删除。
  TiXmlElement* Book = Library->FirstChildElement("Book");
  Book->Clear();
  
  //2). 方式二
  //获取到对应节点后，根节点调用RemoveChild去删除，传参时需要对变量转换一下
  
  // 获取Book节点
  TiXmlElement* Book = Library->FirstChildElement("Book");
  Library->RemoveChild(Book->ToElement());
  ```

4. 删除多个节点

  ```c++
  //当然，如果Book节点不止一个时，如果需要删除特定的几个，可以使用for循环去删除, 判断文本，符合条件就删除
  //调用FirstChildElement方法，传入子节点名字，可以获得第一个相同名字的子节点
  //调用NextSiblingElement方法，传入节点名字，可以获得下一个相同名字的节点
  //用RemoveChild方法，是父节点删除子节点的方法
  
  TiXmlElement* pItem1 = Library->FirstChildElement("Book");
  for (; pItem1 != NULL; ) {
      if (strncmp(pItem1->GetText(), "书本", 6) == 0) {
          // 提前存储删除节点的下一个节点
          TiXmlElement* temporary = pItem1->NextSiblingElement("Book1");
  
          // 删除当前节点，删除后pItem为NULL，如果再继续使用它会报错
          Library->RemoveChild(pItem1->ToElement());
  
          // 所以这里要进行赋值回来
          pItem1 = temporary;
  
      } else {
          // 寻找下一个Book1节点
          pItem1 = pItem1->NextSiblingElement("Book1");
      }
  }
  ```

### 2.删除属性

删除属性直接删除即可，不需要做其他什么操作！

- 调用Attribute方法，传入属性名可以获得属性的值
- 调用RemoveAttribute方法，传入属性名可以进行删除

```c++
// 删除属性
TiXmlElement* pItem = Library->FirstChildElement("Book1");
for (; pItem != NULL; pItem = pItem->NextSiblingElement("Book1")) {
    // 找到属性ID = 1的节点
    if (strcmp(pItem->Attribute("ID"), "1") == 0) {
        // 删除属性为Name的属性
        pItem->RemoveAttribute("Name");
    }
}
```
### 3.删除整个节点

思路和上面删除Book节点一样

```c++
//这里是删除ID属性为2的Book1节点

pItem = Library->FirstChildElement("Book1");
for (; pItem != NULL; ) {
    // 找到属性ID = 2的节点
    if (strcmp(pItem->Attribute("ID"), "2") == 0) {
        // 提前存储删除节点的下一个节点
        TiXmlElement* temporary = pItem->NextSiblingElement("Book1");

        // 删除当前节点，删除后pItem为NULL，如果再继续使用它会报错
        Library->RemoveChild(pItem->ToElement());

        // 所以这里要进行赋值回来
        pItem = temporary;

    } else {
        // 寻找下一个Book1节点
        pItem = pItem->NextSiblingElement("Book1");
    }
}
```
## 5.修改

修改(更改)XML中节点的值

例：将ID属性为3的Book1节点的Price属性值修改为33.33；将ID属性为1的Book1节点的Description子节点值修改为108个没房住，Page修改为999页。

1. 定义一个xml文件类，并读取文件中的xml内容初始化它

2. 获取根节点

3. 修改属性值
  符合条件后使用SetAttribute方法即可进行重新修改属性

  ```c++
  TiXmlElement* pItem = Library->FirstChildElement("Book1");
  for (; pItem != NULL; pItem = pItem->NextSiblingElement("Book1")) {
      // 设置属性
      if (strcmp(pItem->Attribute("ID"), "3") == 0) {
          pItem->SetAttribute("Price", "33.33")
      }
  }
  ```


4. 修改节点值
  首先使用FirstChildElement获得对应节点后，在使用FirstChild获得需要修改的文本指针对象，最后在调用SetValue就可以进行修改了

  ```c++
  TiXmlElement* pItem = Library->FirstChildElement("Book1");
  for (; pItem != NULL; pItem = pItem->NextSiblingElement("Book1")) {
      // 找到属性ID = 1的节点
      if (strcmp(pItem->Attribute("ID"), "1") == 0) {
          // 设置Book1的子节点Description的值
          TiXmlElement* Description = pItem->FirstChildElement("Description");	// 获得<Description>108个没房住</Description>
          TiXmlNode* des = Description->FirstChild();	// 获取元素指针	// 获得存储 108个没房住 的指针
          des->SetValue("108个没房住");	// 重新为其设置值
  
          TiXmlElement *Page = pItem->FirstChildElement("Page");
          TiXmlNode *page = Page->FirstChild();
          page->SetValue("999页");
      }
  ```

## 6.解析

### 1.解析指定节点中值

1. 定义一个xml文件类，并读取文件中的xml内容初始化它

2. 获取根节点

3. 解析Book节点，当然，前面已将Book节点删掉了，所以获取到的Book为NULL所以如果是指针，在使用前最好先判断一下再去使用！

  ```c++
  TiXmlElement* Book = Library->FirstChildElement("Book");
  if (Book) {
  	printf("Book = %s\n\n", Book->GetText());
  }
  ```

4. 解析所有的Book1节点的属性，直接调用Attribute，传入属性名即可获得属性的值。

  ```c++
  /* 当有多个相同名字的节点时，可以使用循环进行读取解析 */
  // 函数FirstChildElement()	:	找到指定名字的元素
  // 函数NextSiblingElement	:	在同一级元素中查找下一个指定名字的元素
  TiXmlElement* pItem = Library->FirstChildElement("Book1");
  if (pItem) {
  	for (; pItem != NULL; pItem = pItem->NextSiblingElement("Book1")) {
  	// 解析属性
  	printf("ID = %s\n", pItem->Attribute("ID"));
  	printf("Name = %s\n", pItem->Attribute("Name"));
  	printf("Price = %s\n", pItem->Attribute("Price"));
  
  	printf("\n\n");
  }
  ```

5. 解析所有的Book1节点的子节点，获得对应解析的节点后，调用GetText可以获得里面的值；

  ```c++
  /* 当有多个相同名字的节点时，可以使用循环进行读取解析 */
  TiXmlElement* pItem = Library->FirstChildElement("Book1");
  if (pItem) {
  	for (; pItem != NULL; pItem = pItem->NextSiblingElement("Book1")) {
  		// 解析Book1的子节点
  		TiXmlElement* Description = pItem->FirstChildElement("Description");
  		printf("Description = %s\n", Description->GetText());
           TiXmlElement *Page = pItem->FirstChildElement("Page");
  		printf("Page = %s\n", Page->GetText());
  		printf("\n\n");
  }
  ```

### 2.解析xml节点值，代码汇总

```c++
void parse_XML() 
{
    printf("\n----- parse_XML -----\n");
    //方式一：当读取有问题时，可以使用下面方式二
    // 定义一个TiXmlDocument类指针
    TiXmlDocument* tinyXmlDoc = new TiXmlDocument(FILE_NAME);
    tinyXmlDoc->LoadFile(TIXML_ENCODING_LEGACY);

    // 读取文档声明信息(也就是xml的头部信息：<?xml version="1.0" encoding="utf-8" ?>)
    TiXmlDeclaration* pDeclar = tinyXmlDoc->FirstChild()->ToDeclaration();
    if (pDeclar != NULL) {
        printf("头部信息：version is %s , encoding is %s\n", pDeclar->Version(), pDeclar->Encoding());
    }

    // 得到文件根节点
    TiXmlElement* Library = new TiXmlElement("Library");
    if (Library) {
        Library = tinyXmlDoc->RootElement();
    }
    // 解析Book节点
    TiXmlElement* Book = Library->FirstChildElement("Book");
    if (Book) {
        printf("Book = %s\n\n", Book->GetText());
    }
    /* 当有多个相同名字的节点时，可以使用循环进行读取解析 */
    // 函数FirstChildElement()	:	找到指定名字的元素
    // 函数NextSiblingElement	:	在同一级元素中查找下一个指定名字的元素
    TiXmlElement* pItem = Library->FirstChildElement("Book1");
    if (pItem) {
        for (; pItem != NULL; pItem = pItem->NextSiblingElement("Book1")) {

            // 解析属性
            printf("ID = %s\n", pItem->Attribute("ID"));
            printf("Name = %s\n", pItem->Attribute("Name"));
            printf("Price = %s\n", pItem->Attribute("Price"));

            // 解析Book1的子节点
            TiXmlElement* Description = pItem->FirstChildElement("Description");
            printf("Description = %s\n", Description->GetText());

            TiXmlElement *Page = pItem->FirstChildElement("Page");
            printf("Page = %s\n", Page->GetText());
            printf("\n\n");
        }
    }
```
