...
分类结果为后续构建规则的选择器（`scripts/Makefile.host` 第 93~133 行）：`host-csingle` 单文件直编（第 98~99 行，HOSTCC），`host-cmulti` 链接多个 .o（第 107~109 行，HOSTLD + `multi_depend` 注入 .o 依赖），`host-cxxmulti` C++ 程序（第 125~127 行，HOSTCXX）。`conf` 正是通过这一行被归入 `host-cmulti`，从而匹配链接规则。
